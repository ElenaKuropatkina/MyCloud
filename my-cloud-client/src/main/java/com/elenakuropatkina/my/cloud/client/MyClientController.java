...
public class MyClientController implements Initializable {
    ...
    public void pressOnUploadBtn(ActionEvent actionEvent) throws Exception {
        if (tfFileName.getLength() > 0) {
            if(Files.size(Paths.get("client_storage/" + tfFileName.getText())) > 3*1024*1024){
               //Network.sendMsg(new CommandMessage(CommandMessage.Command.BIG_FILE_START));
               FileSplit fs = new FileSplit(Paths.get("client_storage/"),  tfFileName.getText());
               for (FileMessage fm : fs.split()){
                   Network.sendMsg(fm);
               }
               Network.sendMsg(new CommandMessage(CommandMessage.Command.BIG_FILE_END));
            } else
            try {
